...
builder.add(test => {
  ...
  return Q.nfcall(
    soyCompiler.compileTemplates.bind(soyCompiler),
    `${__dirname}/assets`
  )
    .then(() => {
      test.deepEqual(
...
builder.add(test => {
  soyCompiler.compileTemplateFiles(
    [`${__dirname}/assets/template1.soy`, `${__dirname}/assets/template2.soy`],
    err => {
      test.ifError(err);
      test.doesNotThrow(assertTemplatesContents.bind(null, test));
      test.done();
...
builder.add(test => {
  ...
  return Q.nfcall(
    ...
    .then(() => {
      test.equal(1, spawnOpts.length);
      return Q.nfcall(
        soyCompilerB.compileTemplates.bind(soyCompilerB, `${__dirname}/assets`)
      );
    })
    .then(() => {
      // Confirm that we re-used the precompiled templates and didn't start a new soy binary.
