...
const closureLibrary = require('obvious-closure-library')
...
SoyVmContext.prototype.loadCompiledTemplateFiles = function (files, callback) {
  ...
  const supportFilePromises = getSupportFilePromises()
  ...
  if (self._contextInitialized) {
    ...
    result = result.then(() => loadFiles(self.getContext(), supportFilePromises)).then(() => {
      self._contextInitialized = true
    })
  }

  result.then(() => loadFiles(self.getContext(), filePromises))
  .then((result) => {
...
function pathsToPromises(paths) {
  return paths.map((path) => Q.nfcall(fs.readFile, path, 'utf8').then((contents) => ({path, contents})))
}
