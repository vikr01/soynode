...
var _obviousClosureLibrary = _interopRequireDefault(require("obvious-closure-library"));
 ...
 * @type {string}
 */
const SOY_UTILS_PATH = _closureTemplates.default['soyutils_usegoog.js'];
/**
...
const CLOSURE_PATHS = ['closure/goog/base.js', 'closure/goog/deps.js', 'closure/goog/debug/error.js', 'closure/goog/dom/nodetype.js', 'closure/goog/string/string.js', 'closure/goog/asserts/asserts.js', 'closure/goog/array/array.js', 'closure/goog/dom/tagname.js', 'closure/goog/object/object.js', 'closure/goog/dom/tags.js', 'closure/goog/string/typedstring.js', 'closure/goog/string/const.js', 'closure/goog/html/safestyle.js', 'closure/goog/html/safestylesheet.js', 'closure/goog/fs/url.js', 'closure/goog/i18n/bidi.js', 'closure/goog/html/safeurl.js', 'closure/goog/html/trustedresourceurl.js', 'closure/goog/html/safehtml.js', 'closure/goog/html/safescript.js', 'closure/goog/html/uncheckedconversions.js', 'closure/goog/structs/structs.js', 'closure/goog/structs/collection.js', 'closure/goog/functions/functions.js', 'closure/goog/math/math.js', 'closure/goog/iter/iter.js', 'closure/goog/structs/map.js', 'closure/goog/structs/set.js', 'closure/goog/labs/useragent/util.js', 'closure/goog/labs/useragent/browser.js', 'closure/goog/labs/useragent/engine.js', 'closure/goog/labs/useragent/platform.js', 'closure/goog/useragent/useragent.js', 'closure/goog/debug/debug.js', 'closure/goog/dom/browserfeature.js', 'closure/goog/dom/safe.js', 'closure/goog/math/coordinate.js', 'closure/goog/math/size.js', 'closure/goog/dom/dom.js', 'closure/goog/structs/inversionmap.js', 'closure/goog/i18n/graphemebreak.js', 'closure/goog/format/format.js', 'closure/goog/html/legacyconversions.js', 'closure/goog/i18n/bidiformatter.js', 'closure/goog/soy/data.js', 'closure/goog/soy/soy.js', 'closure/goog/string/stringbuffer.js'].map(file => _path.default.join(_obviousClosureLibrary.default.dirname, file));

function pathsToPromises(paths) {
  return paths.map(pathToPromise => _q.default.nfcall(_fs.default.readFile, pathToPromise, 'utf8').then(contents => ({
    path: pathToPromise,
    contents
  })));
}
...
const RESET_DELTEMPLATE_REGISTRY_CODE = 'soy.$$DELEGATE_REGISTRY_PRIORITIES_ = {};\n' + 'soy.$$DELEGATE_REGISTRY_FUNCTIONS_ = {};';
/**
...
function SoyVmContext(name, options) {
  ...
  this._templateCache = {};
  this._context = _vm.default.createContext({});
...
SoyVmContext.prototype.get = function (templateName) {
  ...
  if (!this._templateCache[templateName]) {
    ...
    try {
      template = _vm.default.runInContext(templateName, this.getContext(), 'soynode.vm');
    } catch (e) {// Fallthrough
    }
...
function loadFiles(context, filePromises) {
  ...
  function next(result) {
    ...
    _vm.default.runInContext(result.contents, context, result.path);
...
SoyVmContext.prototype.loadCompiledTemplateFiles = function (files, callback) {
  ...
  const self = this; // load the contextJsPaths into the context before the soy template JS

  const filePromises = pathsToPromises(options.contextJsPaths.concat(files));
  const supportedFilePromises = getSupportFilePromises();

  let result = _q.default.resolve(true);

  if (self._contextInitialized) {
    result = _q.default.fcall(() => {
      _vm.default.runInContext(RESET_DELTEMPLATE_REGISTRY_CODE, self.getContext(), 'soynode-reset.');
    });
  } else {
    result = result.then(() => loadFiles(self.getContext(), supportedFilePromises)).then(() => {
      self._contextInitialized = true;
      return null;
    });
  }

  result.then(() => loadFiles(self.getContext(), filePromises)).then(finalResult => {
