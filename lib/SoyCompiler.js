...
const Q = require('q')
const SoyVmContext = require('./SoyVmContext')
const SoyOptions = require('./SoyOptions')
...
SoyCompiler.prototype.compileTemplateFiles = function (files, callback) {
  ...
  const inputDir = this._options.inputDir
  const self = this
  this._maybeUsePrecompiledFiles(outputDir, files)
    .then((dirtyFiles) => {
      self._maybeSetupDynamicRecompile(inputDir, outputDir, files, emitter)
      self._compileTemplateFilesAndEmit(inputDir, outputDir, files, dirtyFiles, emitter)
    })
    .done()
  return emitter
}
...
SoyCompiler.prototype._compileTemplateFilesAsync = function (inputDir, outputDir, allFiles, dirtyFiles) {
  const options = this._options
  let outputPathFormat = path.join(outputDir, '{INPUT_DIRECTORY}', '{INPUT_FILE_NAME}.js')
  ...
  function runCompiler() {
    ...
    const cp = child_process.spawn('java', args, {cwd: inputDir})
    let stderr = ''
...
SoyCompiler.prototype._compileTemplatesAndEmit = function (inputDir, emitter) {
  ...
  findFiles(inputDir, 'soy', (err, files) => {
    ...
    self._maybeUsePrecompiledFiles(outputDir, files)
      .then((dirtyFiles) => {
        self._maybeSetupDynamicRecompile(inputDir, outputDir, files, emitter)
        self._compileTemplateFilesAndEmit(inputDir, outputDir, files, dirtyFiles, emitter)
      })
...
SoyCompiler.prototype._maybeUsePrecompiledFiles = function (outputDir, files) {
  const precompiledDir = this._options.precompiledDir
  if (!precompiledDir) {
  ...
  return Q.resolve(true)
    .then(() => 
      // Return an array of files that don't have precompiled versions.
       Q.all(files.map((file) => self._preparePrecompiledFile(outputDir, precompiledDir, file, vmTypes)
          .then((ok) => ok ? '' : file)))
    )
    .then((dirtyFiles) => {
...
SoyCompiler.prototype._preparePrecompiledFile = function (outputDir, precompiledDir, file, vmTypes) {
  const self = this
  const precompiledFilesOkPromise = Q.all(vmTypes.map((vmType) => {
    ...
    const precompiledFileOkPromise = Q.nfcall(fs.stat, precompiledFileName)
      .then((exists) => {
        if (!exists) {
          return false
        }
  ...
  return precompiledFilesOkPromise
    .then((array) => array.every(Boolean))
}
...
SoyCompiler.prototype._concatOutput = function (outputDir, files, vmType) {
  const options = this._options
  let concatFileName = options.concatFileName
  ...
  const concatenated = files.map((file) => fs.readFileSync(file).toString()).join('')
...
SoyCompiler.prototype._postCompileProcess = function (outputDir, files, vmType) {
  ...
  if (options.loadCompiledTemplates) {
    ...
    return Q.nfcall(this.loadCompiledTemplateFiles.bind(this, templatePaths, {vmType}))
  } 
    return Q.resolve(true)
